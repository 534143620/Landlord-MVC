   ...
   /// <returns></returns>
    ...
    public static bool IsStraight(List<Card> cards)
    {
        if (cards.Count < 5 || cards.Count > 12 )
            return false;
        for (int i = 0; i < cards.Count - 1; i++)  // 因为下面有i+1 所以这边循环要-1 ，不然越界
        {
    ...
    {
        if (cards.Count < 6 || cards.Count %2 != 0)     //牌数小于6，而且要双,不能为单
            return false;

        for (int i = 0; i < cards.Count - 2; i+=2)  // 因为下面有i+1 所以这边循环要-1 ，不然越界
        {
    ...
    {
        if (cards.Count < 6 || cards.Count % 3 != 0)     //牌数小于6，而且是三的倍数
            return false;

        for (int i = 0; i < cards.Count - 3; i += 3)  // 因为下面有i+1 所以这边循环要-1 ，不然越界
        {
            if (cards[i + 1].Cardweight != cards[i].Cardweight)
                return false;
    ...
    public static bool IsThreeAndTwo(List<Card> cards)
    {
        ...
        if (cards[0].Cardweight == cards[1].Cardweight 
           ...
           && cards[0].Cardweight == cards[2].Cardweight)
        {
